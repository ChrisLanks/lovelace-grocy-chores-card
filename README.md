...
| filter_user                  | number/list | **Optional** |          | Only show items assigned to the user(s) with this user_id. Ex: `1` or `[1, 2]` for multiple users. If a user_id map is defined, `current` may be used instead of a number, in which case only items assigned to the current home-assistant user are shown. When used together with `show_unassigned`, see the `show_unassigned` option for combined behavior.                                                                                                                                                                      |
| show_unassigned              | bool        | **Optional** | `false`  | Controls the display of chores or tasks that have no assignee. When `filter_user` is set: if `show_unassigned` is `true`, shows items matching the user filter OR unassigned items; if `show_unassigned` is `false`, only shows items matching the user filter. When `filter_user` is not set: if `show_unassigned` is `false`, hides all unassigned items (only shows assigned items); if `show_unassigned` is `true` or undefined, shows all items (no filtering by assignment). |
